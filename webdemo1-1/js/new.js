(function () {
  ...
  function signAndRequest({ secretId, secretKey, service, host, action, payload, version, region, token }) {
  ...
  recorder.onStop = (audioBuffers) => {
    console.log("录音结束，获取音频数据", audioBuffers);

    // 处理音频数据，例如合并为 Blob 并创建音频 URL
  ...
  btnControl.onclick = function () {
    if (btnStatus === "UNDEFINED" || btnStatus === "CLOSED") {
      ...
      recorder.stop().then((audioBuffer) => {
        ...
        console.log("Base64 录音数据:", base64Audio);
  
        // 上传数据
        uploadData(base64Audio);
      });
    }
  };
})();
